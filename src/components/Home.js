...
const Home = () => {
  ...
  const pTag1 = "font-bold text-2xl text-white";
  const pTag2 = "font-[400] text-xl text-white";
  ...
  return (
    <section className="wrapper h-[100vh] -z-10 ">
      <input
        ...
        type="text"
        placeholder="ENTER YOUR LOCATION (Country, State, City, etc..)"
        className="mx-auto top-[4rem] absolute left-0 right-0 w-1/2 lg:w-[30%] glass border border-gray-500 h-12 outline-none pl-6 text-white placeholder:text-gray-300 placeholder:font-semibold font-semibold "
      />
      <div className="flex justify-between pl-5 w-full mt-[13rem] ">
        <div className="ml-0 xl:ml-20">
          <p className="text-2xl lg:text-3xl text-white font-bold uppercase">
            {data.name}
          </p>
          {data.name ? (
            <p className="lg:text-9xl text-8xl font-bold text-white">
      ...
      <div className="w-[90%] lg:w-[80%] xl:w-[70%] h-32 absolute bottom-6 mx-auto left-0 right-0 glass flex justify-around items-center">
        {data.name ? (
